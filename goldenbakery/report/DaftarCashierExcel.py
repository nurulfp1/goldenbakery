...
class PartnerXlsx(models.AbstractModel):
    _name = 'report.goldenbakery.report_cashier_xlsx'
    _inherit = 'report.report_xlsx.abstract'
    tgl_lap = fields.Date.today()

    def generate_xlsx_report(self, workbook, data, cashier):
        sheet = workbook.add_worksheet('Daftar Employee cashier')
        bold = workbook.add_format({'bold': True})
        sheet.write(0, 0, str(self.tgl_lap))
        sheet.write(2, 0, 'ID Cashier')
        ...
        for obj in cashier: 
            ...
            sheet.write(row, col, obj.id_cashier)
