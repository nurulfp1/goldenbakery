...
class PartnerXlsx(models.AbstractModel):
    _name = 'report.goldenbakery.report_chef_xlsx'
    _inherit = 'report.report_xlsx.abstract'
    tgl_lap = fields.Date.today()

    def generate_xlsx_report(self, workbook, data, chef):
        sheet = workbook.add_worksheet('Daftar Employee Chef')
        bold = workbook.add_format({'bold': True})
        sheet.write(0, 0, str(self.tgl_lap))
        sheet.write(2, 0, 'ID Chef')
        ...
        for obj in chef: 
            ...
            sheet.write(row, col, obj.id_chef)
